...
def signup(request):
    if request.method == 'POST':
        if 'otp' in request.POST:  # Check if OTP is being submitted
            ...
            # Parse OTP generation time as a timezone-aware datetime
            otp_generation_time = timezone.datetime.fromisoformat(signup_info.get('otp_generation_time'))
            current_time = timezone.now()
            otp_validity_period = timezone.timedelta(minutes=5)  # Set OTP validity period here
            ...
            # Save the OTP, timestamp, and other user info in session
            request.session['signup_info'] = {
                'username': username,
                'email': email,
                'password': password,
                'otp': otp,
                'otp_generation_time': otp_generation_time.isoformat()  # Store as ISO format string
            }
            ...
            return redirect('signup')  # Keep user on the signup page to enter OTP
